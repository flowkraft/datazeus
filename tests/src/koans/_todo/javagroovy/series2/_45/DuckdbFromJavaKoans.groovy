package datazeus.javagroovy.series2._45
...
/**
 * ╔══════════════════════════════════════════════════════════════════════╗
 * ║  JVM KOANS — Java & Groovy for Data · Series 2 · 45
 * ╚══════════════════════════════════════════════════════════════════════╝
 *
 * DuckDB from Java — An Analytics Engine Inside Your Process
 *
 *     zeus.bat koans javagroovy series2 _45     (Windows)
 *     ./zeus.sh koans javagroovy series2 _45    (macOS/Linux)
 *
 ...
 * GOAL: run analytical SQL in your own process, with no server.
 * CONSTRUCTS: duckdb_jdbc, in-memory vs file, reading CSV/Parquet directly with read_csv_auto
 * and read_parquet, and joining that against your own data.
 * Same engine the koans run on, so the reader is already using it — say so.
 */
@Stepwise // walk them in order — once one fails, the rest wait
class DuckdbFromJavaKoans extends JvmKoanBase {
