package datazeus.javagroovy.series1._25
...
/**
 * ╔══════════════════════════════════════════════════════════════════════╗
 * ║  JVM KOANS — Java & Groovy for Data · Series 1 · 25
 * ╚══════════════════════════════════════════════════════════════════════╝
 *
 * java.time — Working With Dates, Times and Time Zones Correctly
 *
 *     zeus.bat koans javagroovy series1 _25     (Windows)
 *     ./zeus.sh koans javagroovy series1 _25    (macOS/Linux)
 *
 ...
 * GOAL: parse, store and compare dates without silently shifting them a day.
 * CONSTRUCTS: Instant vs LocalDate vs LocalDateTime vs ZonedDateTime, ZoneId, DateTimeFormatter.
 * KOAN (predict): a timestamp near midnight UTC, read in two zones — which day does the daily
 * report put it on? One right answer, deterministic, and it is exactly the bug that makes a
 * report disagree with itself between two offices.
 */
@Stepwise // walk them in order — once one fails, the rest wait
class DatesAndTimeKoans extends JvmKoanBase {
